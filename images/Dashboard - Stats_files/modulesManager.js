(function(global) {
    ...
    function getModuleProductionPath(moduleName, options) {
        ...
        if (commonProps['modules.resources.cdn'] && serverName === '') {
            serverName = commonProps['common.resources.folder'];
        }
        var folderName = options.folderName || moduleName;
        return serverName + '/editor/apps/modules/' + folderName + '/' + moduleName + '.js';
    }
        ...
        return new Promise(function loadModule(resolve, reject) {
            if (getModuleFromCache(moduleName)) {
                ...
                getExternals().then(function onExternalScriptPromise(result) {
                    var serverForResources = commonProps['server.for.resources'];
                    var clientModulesFallback = commonProps['client.modules.fallback'];
                    ...
                    if (window.isMobileDevice && !window.define && !!window._define) {
                    ...
                    requireAsPromise(requireModuleName)
                        .catch(function onFirstRequireFail(err) {
                            if (!serverForResources || !err || err.requireType !== 'scripterror') {
                                return Promise.reject(err);
                            } else {
                                console.error('Unable to fetch module: ' + moduleName + ', retrying from relative');
                        ...
                        .catch(function onSecondRequireFail(err) {
                            if (!clientModulesFallback || !err || err.requireType !== 'scripterror') {
                                return Promise.reject(err);
                            }
                            console.error('Unable to fetch module: ' + moduleName + ', retrying from master');
                        ...
                        .catch(function onFetchModuleFail(err) {
                            if (err && err.requireType === 'scripterror') {
                                console.error('Unable to fetch module: ' + moduleName);
                            }
                            reject(err);
                        });
                });
    ...
    (function _init() {
        ...
        if (loadedModules) {
            Object.keys(loadedModules).forEach(function(loadedModuleName) {
                setModuleInCache(loadedModuleName, loadedModules[loadedModuleName]);
            });
        }
    })();
